...
uint32_t g_ccr_val1 = 500; /* 比较值变值 */
uint8_t g_run_flag1 = 0;   /* 标志位 */

STEPPER_MOTOR g_stepper2;
uint32_t g_count_val2 = 0; /* 计数值 */
uint32_t g_ccr_val2 = 500; /* 比较值变值 */
uint8_t g_run_flag2 = 0;   /* 标志位 */
...
void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if (htim->Instance == TIM4)
  {
    ...
    if (count1 % 2 == 0)
    {
      ...
      {
        printf("angle count: %d\r\n", (int)(g_stepper1.add_pulse_count * MAX_STEP_ANGLE)); /* 打印累计转动了多少角度 */
  ...
  if (htim->Instance == TIM1)
  {
    ...
    if (count2 % 2 == 0)
    {
      ...
      {
        printf("angle count: %d\r\n", (int)(g_stepper2.add_pulse_count * MAX_STEP_ANGLE)); /* 打印累计转动了多少角度 */
        stepper_stop(STEPPER_MOTOR_1);                                                     /* 停止接口一输出 */
        ST1_EN(EN_OFF);
        g_run_flag2 = 0;
      }
